...
df_overall = df %>% group_by(Condition, Metrics)  %>%
  summarise(Mean = round(mean(Metrics_num),4)*100,
            SD = round(sd(Metrics_num),4)*100,
            .groups = 'drop')

library(reshape2) 
df_overall <- reshape2::melt(df_overall) 
df_overall <- dcast(df_overall, Condition ~...) 
write.csv(df_overall, "../plots/df_overall.csv", row.names = F)
stargazer::stargazer(df_overall, summary = F, rownames = F, type = 'text', out = '../plots/df_overall.txt')
...
df_overall_t1dvsctl = df1 %>% group_by(Condition, Metrics)  %>%
  summarise(Mean = round(mean(Metrics_num),4)*100,
            SD = round(sd(Metrics_num),4)*100,
            .groups = 'drop')

library(reshape2) 
df_overall_t1dvsctl <- reshape2::melt(df_overall_t1dvsctl) 
df_overall_t1dvsctl <- dcast(df_overall_t1dvsctl, Condition ~...) 
write.csv(df_overall_t1dvsctl, "../plots/df_overall_t1dvsctl.csv", row.names = F)
stargazer::stargazer(df_overall_t1dvsctl, summary = F, rownames = F, type = 'text', out = '../plots/df_overall_t1dvsctl.txt')
...
df_overall_t1dvsaab = df2 %>% group_by(Condition, Metrics)  %>%
  summarise(Mean = round(mean(Metrics_num),4)*100,
            SD = round(sd(Metrics_num),4)*100,
            .groups = 'drop')

library(reshape2) 
df_overall_t1dvsaab <- reshape2::melt(df_overall_t1dvsaab) 
df_overall_t1dvsaab <- dcast(df_overall_t1dvsaab, Condition ~...) 
write.csv(df_overall_t1dvsaab, "../plots/df_overall_t1dvsaab.csv", row.names = F)
stargazer::stargazer(df_overall_t1dvsaab, summary = F, rownames = F, type = 'text', out = '../plots/df_overall_t1dvsaab.txt')
...
df_overall_aabvsctl = df3 %>% group_by(Condition, Metrics)  %>%
  summarise(Mean = round(mean(Metrics_num),4)*100,
            SD = round(sd(Metrics_num),4)*100,
            .groups = 'drop')

library(reshape2) 
df_overall_aabvsctl <- reshape2::melt(df_overall_aabvsctl) 
df_overall_aabvsctl <- dcast(df_overall_aabvsctl, Condition ~...) 
write.csv(df_overall_aabvsctl, "../plots/df_overall_aabvsctl.csv", row.names = F)
stargazer::stargazer(df_overall_aabvsctl, summary = F, rownames = F, type = 'text', out = '../plots/df_overall_aabvsctl.txt')
##################################################################################
