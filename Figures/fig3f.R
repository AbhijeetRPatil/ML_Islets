...
## Acinar
################################################################
Acinar <- read.csv("./acinar_xgb_aabvsctrl_RRA_GL.csv")
Acinar <- Acinar$Genes
Acinar <- as.character(unique(Acinar))
################################################################
## Alpha
################################################################
Alpha <- read.csv("./alpha_xgb_aabvsctrl_RRA_GL.csv")
Alpha <- Alpha$Genes
Alpha <- as.character(unique(Alpha))
################################################################
## Beta
################################################################
Beta <- read.csv("./beta_xgb_aabvsctrl_RRA_GL.csv")
Beta <- Beta$Genes
Beta <- as.character(unique(Beta))
################################################################
## Ductal
################################################################
Ductal <- read.csv("ductal_xgb_aabvsctrl_RRA_GL.csv")
Ductal <- Ductal$Genes
Ductal <- as.character(unique(Ductal))
################################################################
## Delta
################################################################
Delta <- read.csv("delta_xgb_aabvsctrl_RRA_GL.csv")
Delta <- Delta$Genes
Delta <- as.character(unique(Delta))
################################################################
## Immune
################################################################
Immune <- read.csv("immune_xgb_aabvsctrl_RRA_GL.csv")
Immune <- Immune$Genes
Immune <- as.character(unique(Immune))
################################################################
## Endothelial
################################################################
Endothelial <- read.csv("endothelial_xgb_aabvsctrl_RRA_GL.csv")
Endothelial <- Endothelial$Genes
Endothelial <- as.character(unique(Endothelial))
################################################################
## Stellates
################################################################
Stellates <- read.csv("stellates_xgb_aabvsctrl_RRA_GL.csv")
...
write.csv(group1_rlfs_list_fin, "AABvsCTL_CellType_RRA.csv")
####################################################################################################################
...
# tiff(file="Venn_aab_CTL_Upset.png",
...
pdf(file="Venn_AAB_CTL_Upset.pdf", width=10, height=6)
upset(
  mat0,
  # sets = data.names,
  c("Acinar", "Alpha", "Beta", "Ductal", "Delta", "Immune", "Endothelial", "Stellates", "RRA_Combined"),
